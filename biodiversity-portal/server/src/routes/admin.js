...
router.get("/dashboard", async (_req, res) => {
  ...
  return res.json({ success: true, data: { users, activeUsers, species, endemicSpecies, observations, pendingObservations, identifications } });
});
...
router.patch("/users/:id/role", async (req, res) => {
  const role = z.enum(["user", "admin", "researcher"]).safeParse(req.body.role);
...
router.get("/audit-logs", async (_req, res) => res.json({ success: true, data: { logs: await AuditLog.find().sort({ createdAt: -1 }).limit(100).populate("adminUserId", "name email").lean() } }));
